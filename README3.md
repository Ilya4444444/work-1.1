...
Как работает: Внешний цикл for (int i = 0; i < n - 1; i++) определяет количество проходов по массиву. Переменная swapped отслеживает, были ли обмены на текущей итерации. Внутренний цикл for (int j = 0; j < n - i - 1; j++) проходит по неотсортированной части массива. Условие if (arr[j] > arr[j + 1]) проверяет соседние элементы, и если они в неправильном порядке, происходит обмен через временную переменную temp. Флаг swapped = true указывает, что была произведена перестановка. Проверка if (!swapped) break оптимизирует алгоритм: если на очередном проходе не было ни одного обмена, массив уже отсортирован и можно досрочно завершить работу.
...
Как работает: Внешний цикл for (int gap = n / 2; gap > 0; gap /= 2) определяет последовательность шагов, начиная с n/2 и уменьшая вдвое на каждой итерации, пока шаг не станет равным 1. Для каждого шага gap применяется модифицированная сортировка вставками. Внутренний цикл for (int i = gap; i < n; i++) проходит по элементам, начиная с позиции gap. Переменная temp = arr[i] сохраняет текущий элемент. Третий цикл for (j = i; j >= gap && arr[j - gap] > temp; j -= gap) сдвигает элементы, находящиеся на расстоянии gap, вправо, пока не найдется правильная позиция для вставки temp. Затем arr[j] = temp помещает элемент на найденное место.
...
Как работает: Цикл for (int i = 0; i < arr.length; i++) последовательно перебирает все элементы массива от начала до конца. На каждой итерации условие if (arr[i] == target) проверяет, совпадает ли текущий элемент с искомым значением. Если совпадение найдено, функция немедленно возвращает индекс i. Если цикл завершается без нахождения элемента, функция возвращает -1, указывая на то, что целевой элемент отсутствует в массиве.
...
Как работает: Инициализируются границы поиска: left = 0 и right = arr.size() - 1. Цикл while (left <= right) продолжается, пока границы не пересекутся. На каждой итерации вычисляется средний индекс mid = left + (right - left) / 2 (защита от переполнения). Затем проверяется:

- Если arr[mid] == target - элемент найден, возвращается индекс mid
- Если arr[mid] < target - поиск продолжается в правой половине: left = mid + 1
- Если arr[mid] > target - поиск продолжается в левой половине: right = mid - 1

Если цикл завершился без нахождения элемента (left > right), возвращается -1.
...
Как работает: ВИнициализируются границы поиска: low = 0 и high = arr.size() - 1. Условие while (low <= high && target >= arr[low] && target <= arr[high]) проверяет, что границы валидны и цель находится в диапазоне массива. Ключевое отличие - формула вычисления позиции: int pos = low + ((double)(high - low) / (arr[high] - arr[low])) * (target - arr[low]). Она предсказывает позицию цели на основе её значения относительно минимального и максимального элементов в текущем отрезке.

Далее как в бинарном поиске:

- Если arr[pos] == target - элемент найден
- Если arr[pos] < target - поиск продолжается справа: low = pos + 1
- Если arr[pos] > target - поиск продолжается слева: high = pos - 1
...
Как работает: Сначала генерируются числа Фибоначчи: fibMMm2 = 0 (F(k-2)), fibMMm1 = 1 (F(k-1)), fibM = fibMMm2 + fibMMm1 (F(k)). Цикл while (fibM < n) находит наименьшее число Фибоначчи, большее или равное размеру массива. Переменная offset = -1 отслеживает смещение в массиве. Основной цикл while (fibM > 1) делит массив используя числа Фибоначчи. Индекс i = min(offset + fibMMm2, n-1) вычисляет позицию для сравнения.

В зависимости от сравнения arr[i] с target:

- Если arr[i] < target - поиск продолжается в правой части, обновляются числа Фибоначчи и смещение
- Если arr[i] > target - поиск продолжается в левой части, обновляются числа Фибоначчи
- Если равны - элемент найден

Последняя проверка if (fibMMm1 == 1 && arr[offset + 1] == target) обрабатывает оставшийся элемент.
